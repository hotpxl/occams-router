...
Controller::Controller(bool debug) : debug_{debug}, current_window_size_{1} {}
...
void Controller::ack_received(std::uint64_t sequence_number_acked,
  ...
  if (kHighWatermark < timestamp_ack_received - send_timestamp_acked) {
    if (1 < current_window_size_) {
      --current_window_size_;
    }
  } else if (timestamp_ack_received - send_timestamp_acked < kLowWatermark) {
    ++current_window_size_;
  }
}
